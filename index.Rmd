...
for $X = X_1, X_2, \dots, X_p$ independent and $Y$ dependent.
...
- Can not distinguish estimates and hard to make variable selections based on p-values etc
- OK for prediction modelling
- Autocorrelation present for modelling longer PROM-series (1, 6 and 10 years). 
...
- `byear` (if included) might be categorized (if that would makes sense) as: baby-boomers, generation X, MTV-generation, millenials etc. Ordinal scale (think!)?
...
- Assume that $\beta_4 = \beta_5 \approx 0$ (with $p = (.4, .6)$ let's say). 
